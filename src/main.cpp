...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice)
    {
      ...
      if ( known_found )
      {
        ...
        char out_json[64];
        sprintf(out_json, "{\"Temp\": %.02f,\"Humi\": %.02f,\"Batt\": %d, \"RSSI\": %d}", temp, humi, batt, rssi); 
...
void pubSensorData( TimerHandle_t pxTimer )
{
...
void bleScan( TimerHandle_t pxTimer )
{
...
void setup()
{
  ...
  readSensorTimer    = xTimerCreate("ReadTimer", pdMS_TO_TICKS(10 * 60 * 1000), pdTRUE, (void*)1, &pubSensorData);
  BLEscanTimer       = xTimerCreate("BLEScanTimer", pdMS_TO_TICKS(10 * 1000), pdTRUE, (void*)2, &bleScan);

  if( readSensorTimer == NULL )
    Serial.println("Did not create timer");
  
  else
  {
    if( xTimerStart( readSensorTimer, 0 ) != pdPASS )
    {
      Serial.println("The timer could not be set into the Active state.");
    }
    else
      Serial.println("Start Timer alright");
  }
  
  xTimerStart(BLEscanTimer, 0);
